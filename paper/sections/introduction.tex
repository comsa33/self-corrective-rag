...
A complex 4-hop question receives the same number of refinement iterations as a simple 2-hop question, regardless of whether the evidence gaps have been resolved.
...
We propose \ours{} (\textbf{T}ool-\textbf{A}ugmented \textbf{R}etrieval \textbf{A}gent), a framework that addresses both limitations through two key design decisions.
...
Using $n = 200$ samples per dataset with paired bootstrap significance testing, we demonstrate that the agentic approach achieved statistically significant improvement over all baselines on 2WikiMultiHopQA (F1 $+0.089$, $p < 0.001$), with consistent positive trends on MuSiQue ($+0.039$).
Cross-model validation with gpt-5-mini revealed a model--pipeline interaction effect---\emph{Reasoning Model Refusal Asymmetry}---where reasoning-oriented models apply excessive evidentiary standards in agentic pipelines, providing practical guidance for model-aware pipeline selection.
DSPy's typed Signatures alone improved F1 by 0.071--0.164 over manual prompts on three of four datasets, with additional gains from automatic optimization.
...
\begin{enumerate}
    ...
    \item We implement the entire pipeline as a DSPy declarative program and demonstrate that typed Signatures alone provide the single largest improvement in our experiments (F1 $+0.075$--$0.190$), with automatic optimization yielding further dataset-dependent gains.

    \item We provide rigorous empirical evaluation ($n = 200$ with bootstrap significance testing) across four datasets, five pipeline variants, and two model families, including ablation studies on tool configurations, identification of boundary conditions (retrieval space saturation) and model--pipeline interaction effects (Reasoning Model Refusal Asymmetry), and quantification of the latency--quality trade-off.
\end{enumerate}
