...
Evaluated on $n = 200$ samples per dataset with paired bootstrap significance testing, results demonstrate that the agentic approach achieves statistically significant improvement over all baselines on 2WikiMultiHopQA (F1 $+0.089$, $p < 0.001$), with consistent positive trends on MuSiQue ($+0.039$).
Cross-model validation with gpt-5-mini reveals a model--pipeline interaction effect---\emph{Reasoning Model Refusal Asymmetry}---where reasoning-oriented models apply excessive evidentiary standards in agentic pipelines, providing practical guidance for model-aware pipeline selection.
DSPy's typed Signatures alone improve F1 by 0.075--0.190 over manual prompts on three of four datasets, with additional gains from automatic optimization.
