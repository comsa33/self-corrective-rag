...
Recent work improves retrieval quality through query rewriting~\cite{ma2023query}, hypothetical document embeddings (HyDE)~\cite{gao2023hyde}, LLM-augmented dense retrieval~\cite{peng2025sptar,silva2024improving}, hybrid retrieval combining dense and sparse methods via Reciprocal Rank Fusion (RRF)~\cite{cormack2009rrf}, and document chunking strategies tailored to specific domains~\cite{zhao2025metachunking,jimeno2024financial}.
...
\textbf{\ours{}}  & \textbf{Adaptive} & \textbf{4+2 tools} & \textbf{DSPy}  & \checkmark & \textbf{ReAct}    \\
\bottomrule
\end{tabular}
}% end resizebox
\end{table*}
...
Search-o1~\cite{li2025searcho1} integrates agentic search into reasoning chains of large reasoning models, dynamically retrieving knowledge when the model encounters uncertainty during extended reasoning.
PlanRAG~\cite{lee2024planrag} introduces a plan-then-retrieve strategy where the LLM first generates an analysis plan before issuing retrieval queries, targeting decision-making over structured databases.
Earlier work explores multi-step retrieval agents that decompose complex queries~\cite{khot2023decomposed}, perform iterative search, and synthesize evidence from multiple sources.
However, most existing agentic RAG systems focus on a single retrieval tool with fixed orchestration logic.

\ours{} differs in providing a diverse tool set (four core, two domain-adaptive) with autonomous tool selection through ReAct reasoning, combined with a declarative DSPy implementation that enables automatic prompt optimization---a combination not explored in prior agentic RAG work.
...
Our work demonstrates that DSPy's structured prompting provides substantial benefits even without optimization ($+0.075$--$0.190$ F1), with additional gains from automatic optimization that are dataset-dependent.
