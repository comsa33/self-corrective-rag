...
Early approaches use a single retrieval pass with dense retrievers such as DPR~\cite{karpukhin2020dense} and ColBERT~\cite{khattab2020colbert}, or sparse methods like BM25~\cite{robertson2009bm25}.
...
Recent work improves retrieval quality through query rewriting~\cite{ma2023query}, hypothetical document embeddings (HyDE)~\cite{gao2023hyde}, and hybrid retrieval combining dense and sparse methods via Reciprocal Rank Fusion (RRF)~\cite{cormack2009rrf}.
...
Broader agent frameworks have also emerged: HuggingGPT~\cite{shen2023hugginggpt} orchestrates specialized AI models as tools, and AgentBench~\cite{liu2024agentbench} provides a benchmark for evaluating LLMs as autonomous agents across diverse environments.
