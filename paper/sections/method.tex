...
This section presents \ours{}, our agentic self-corrective RAG framework.
...
% \caption{Overall architecture of \ours{}. The pipeline consists of four stages: (1) query preprocessing, (2) ReAct agentic refinement with six tools, (3) passage merging, and (4) answer generation. The agent autonomously selects tools based on reasoning about retrieval quality.}
% \label{fig:architecture}
% \end{figure}

Figure~\ref{fig:architecture} illustrates the \ours{} pipeline, which consists of four stages:
...
\begin{enumerate}
    ...
    \item \textbf{ReAct Agentic Refinement.} The preprocessed query is passed to a ReAct agent equipped with six tools (\S\ref{subsec:agent}). The agent iteratively searches, decomposes, evaluates, and inspects passages through autonomous reasoning. This stage replaces the fixed for-loop in prior work.
    ...
    \item \textbf{Answer Generation.} A ChainOfThought generator (DSPy \texttt{QnAGenerateSignature}) produces the final answer conditioned on the merged passages, with adaptive response formatting based on question type (factoid, analytical, yes/no).
\end{enumerate}
...
The agent operates through a DSPy \texttt{Signature} (\texttt{AgenticRefinementSignature}) that specifies typed inputs and outputs.
...
\begin{enumerate}
    \item \textbf{\texttt{search\_passages}$(q, \mathcal{K})$}: Executes hybrid retrieval (FAISS + BM25 with RRF fusion) and returns passage previews (first 200 characters) to conserve context. This is the primary retrieval tool, called 3--4 times per question on average.
    ...
    \item \textbf{\texttt{evaluate\_passages}$(q, \mathcal{P})$}: Assesses passage quality on four dimensions---Relevance (0--30), Coverage (0--25), Specificity (0--25), and Sufficiency (0--20)---producing a total score $s \in [0, 100]$ with an action decision (\texttt{output}/\texttt{refine}) and refinement feedback (suggested query, keywords to add/remove). A mandatory fallback ensures this tool is called at least once per question, even if the agent omits it.
...
Additionally, a \texttt{calculate} tool handles numerical computations for domain-specific questions (\eg, financial calculations).
...
This ensures the agent eventually terminates with the best available passages rather than endlessly searching for marginally better results.

\paragraph{Structured Feedback.}
When the evaluation score falls below the threshold, the evaluation tool provides targeted feedback---suggested queries, keywords to add/remove---that directly informs the agent's next search action.
...
\begin{itemize}
    \item \textbf{PreprocessSignature}: $(\texttt{question}, \texttt{history}) \rightarrow (\texttt{rephrased\_question}, \texttt{keywords}, \texttt{topic})$
    \item \textbf{EvaluationSignature}: $(\texttt{question}, \texttt{passages}, \texttt{retry\_count}) \rightarrow (\texttt{scores}_{4D}, \texttt{action}, \texttt{feedback})$
    \item \textbf{QnAGenerateSignature}: $(\texttt{question}, \texttt{passages}) \rightarrow (\texttt{answer}, \texttt{footnotes})$
    \item \textbf{DecomposeQuerySignature}: $(\texttt{question}) \rightarrow (\texttt{is\_multi\_hop}, \texttt{sub\_questions})$
    \item \textbf{AgenticRefinementSignature}: $(\texttt{question}, \texttt{query}, \texttt{keywords}, \tau) \rightarrow (\texttt{passages}, \texttt{action})$
\end{itemize}
...
\begin{itemize}
    \item \texttt{ChainOfThought}~\cite{wei2022chain}: Wraps PreprocessSignature and QnAGenerateSignature to elicit step-by-step reasoning before outputs.
    \item \texttt{Predict}: Wraps EvaluationSignature for direct structured output.
    \item \texttt{ReAct}: Wraps AgenticRefinementSignature with tool integration, implementing the Thought--Action--Observation loop.
\end{itemize}
...
\begin{equation}
    ...
    \label{eq:rrf}
\end{equation}
where $k = 60$ is a constant and $\text{rank}_r(d)$ is the rank of document $d$ in retriever $r$'s result list.
The fused ranking is used as the final retrieval output, with $k_{\text{top}} = 50$ results returned per query.
