...
\textbf{HotpotQA}~\cite{yang2018hotpotqa} is a two-hop QA dataset over Wikipedia, where each question requires reasoning over two supporting paragraphs mixed with distractor paragraphs.
...
We do not include Self-RAG~\cite{asai2024selfrag} or Adaptive-RAG~\cite{jeong2024adaptiverag} as direct baselines because they require additional trained components (Self-RAG: a fine-tuned model with reflection tokens; Adaptive-RAG: a trained complexity classifier) that would confound the comparison of refinement strategies.
All our baselines share the same retrieval infrastructure, generation model, and closed-corpus setting to isolate the effect of refinement mechanism design.
...
\begin{itemize}
    ...
    \item \textbf{Token-level F1}: Harmonic mean of token-level precision and recall between the normalized prediction and reference, following the standard SQuAD evaluation protocol. This awards partial credit for incomplete but overlapping answers.
...
The quality threshold is $\tau = 40$ with progressive threshold relaxation $\delta = 5$ per evaluation attempt (minimum effective threshold: 20).
The maximum retry count for the evaluation tool is $T = 3$.
The maximum number of accumulated passages is $M = 30$.
...
All code, configurations, and experiment scripts are available at \url{https://github.com/comsa33/self-corrective-rag}.
