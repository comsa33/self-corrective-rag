...
- **RQ3**: 4D eval as tool — RLM+4D vs RLM+1D vs RLM w/o eval
...
agentic_rag/
  ...
  config/
    __init__.py
    settings.py              — Pydantic settings with ablation flags + RLMSettings
  ...
  signatures/
    ...
    rlm_refinement.py        — RLM agentic refinement signature (C1)
  ...
  pipeline/
    ...
    agentic.py               — AgenticRAGPipeline (RLM, PROPOSED METHOD)
...
configs/
  base.yaml                  — Shared defaults (model, retrieval, evaluation, rlm)
...
tests/
  ...
  test_rlm_refinement.py     — RLM tools, section/term index tests
